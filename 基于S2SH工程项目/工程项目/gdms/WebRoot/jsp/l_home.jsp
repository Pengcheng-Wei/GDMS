...
<html lang="en" style="height:100%">
  ...
  <body style="height:100%;overflow: hidden;">
			...
			<div class="ue-vmenu">
				<ul>
					...
					<li>
						<a>文档填写</a>
						<ul>
							<li><a data-role="leaf" onclick="addAssBook()">任务书</a></li>
							<li><a data-role="leaf" onclick="addRePro()">开题报告</a></li>
					...
					<li><a data-role="leaf" onclick="getFinalGradeAll()">论文总成绩查询</a></li>
				</ul>
			</div>
		</div>
		</div>
   ...
   <script type="text/javascript">
	...
	function getFinalGradeAll()
	{
		$("#mainFrame").attr({src:"<%=request.getContextPath()%>/doc_getFinalGradeAll.action"});
	}
	function doReply()
	{
		$.sticky(
	...
	function addAssBook()
	{
		$("#mainFrame").attr({src:"<%=request.getContextPath()%>/doc_assBookForLead.action"});
	}
	function addRePro()
	{
		$("#mainFrame").attr({src:"<%=request.getContextPath()%>/doc_reProForLead.action"});
	}
	function doCheck()
	{
		$("#mainFrame").attr({src:"<%=request.getContextPath()%>/sub_getAll.action"});
		
	}
	function updateReAu()
	{
		$("#mainFrame").attr({src:"<%=request.getContextPath()%>/doc_reviewRepForLeadIndex"});
	}
