...
<%
String j_auth_action=request.getContextPath()+"/sub_getSubLead.action";
%>
<!DOCTYPE html>
<html>
<head>
...
// 查询
function search() {
	document.forms[0].action ="<%=j_auth_action%>?personId=" +${person.id}+"&currentPage="+${pageBean.currentPage}+"&pageSize=${pageBean.pageSize}";  //将表单数据提交给地址j_auth_action
	document.forms[0].submit();
}
function doNextPage(){
	
	document.forms[0].action ="<%=j_auth_action%>?personId=" +${person.id}+"&currentPage=" +${pageBean.nextPage }+ "&pageSize=" + ${pageBean.pageSize};  //将表单数据提交给地址j_auth_action
	document.forms[0].submit();
}
function doFristPage()
{
	document.forms[0].action="<%=j_auth_action%>?personId=" +${person.id}+"&currentPage=" + ${pageBean.firstPage }+"&pageSize="+ ${pageBean.pageSize};
	document.forms[0].submit();
}
function doLastPage()
{							//?currentPage=${pageBean.totalPage }&pageSize=${pageBean.pageSize}
	document.forms[0].action="<%=j_auth_action%>?personId=" +${person.id}+"&currentPage=" + ${pageBean.totalPage }+"&pageSize="+ ${pageBean.pageSize};
	document.forms[0].submit();
}
function doPrePage()
{							//?currentPage=${pageBean.prePage}&pageSize=${pageBean.pageSize}
	document.forms[0].action="<%=j_auth_action%>?personId=" + ${person.id} +"&currentPage=" + ${pageBean.prePage}+"&pageSize="+ ${pageBean.pageSize};
	document.forms[0].submit();
}
</script>
</head>
	...
	<div class="container">
			   ...
			   <c:forEach items="${pageBean.data}" var="subject" varStatus="varSta">
					...
					<c:choose>
			    	 	 	<c:when test="${subject.direcOpt == '审核通过' }">
			    	 	 		<td style="color:  #32CD32">${subject.direcOpt}</td>
			    	 	 	</c:when>
			    	 	 	<c:otherwise>
			    	 	...
			    	 	<td><a href="${pageContext.request.contextPath}/sub_seeSub.action?sub_id=${subject.sub_id}">查看详情</a></td>
			    	 </tr>
	    		  </c:forEach>
	    		      	 <tr>
	 ...
	 <script type="text/javascript">
  	...
  	function changePageSize(){
  		...
  		var url = "${pageContext.request.contextPath}/sub_getSubLead.action?personId=${person.id}&currentPage=${pageBean.currentPage}&pageSize="+pageSize;
  	...
  	function changePage(){
  		...
  		var url = "${pageContext.request.contextPath}/sub_getSubLead.action?personId=${person.id}&currentPage="+curPage+"&pageSize=${pageBean.pageSize}";
  		window.location.href=url;
  	}
  </script>
</body>
