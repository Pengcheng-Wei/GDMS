...
<script type="text/javascript">
	function updateRePro(rePro_id,stu_id)
	{
		if(rePro_id==null|| rePro_id=="")
		{
			...
			="${pageContext.request.contextPath}/doc_updateInCheckIndex.action?stu_id="+stu_id+"&teach_id=${teacher.id}";	
		  ...
		  <tbody>
			   <c:forEach items="${stuResultList}" var="student" varStatus="varSta">
			    	 <tr>
			    	 	...
			    	 	<td> 
			    	 	<button type="button" class="btn ue-btn" onclick="updateRePro('${student.interimCheck.inCheck_id}','${student.id}')"> <span class="fa fa-search"></span>填写中期检测意见</button>
			    	 	</td>
			    	 </tr>
	    		  </c:forEach>
